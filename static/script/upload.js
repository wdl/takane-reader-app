...
function uploadProgress(event, key) {
    const tr = document.querySelector(`#upload > footer > table > tbody > tr[data-key="${key}"]`)
...
function uploadError(key = null) {
    if(key) {
        ...
        const trs = document.querySelectorAll(`#upload > footer > table > tbody > tr:not(.title)`)
...
document.addEventListener('DOMContentLoaded', (e) => {
    ...
    ;['drag', 'dragstart', 'dragend', 'dragover', 'dragenter', 'dragleave', 'drop'].forEach(eventName => divDropZone.addEventListener(eventName, (event) => uploadDragDropPrevent(event)))
    ;['dragover', 'dragenter'].forEach(eventName => divDropZone.addEventListener(eventName, uploadDragOn))
    ;['dragend', 'dragleave', 'drop'].forEach(eventName => divDropZone.addEventListener(eventName, uploadDragOff))
})
