async function listPageLoad() {
    try {
        ...
        spanShowEmailHere.innerText = localStorage.getItem('email')
    } catch(e) {
...
function listMainListing(list) {
    ...
    for(item of list) {
        ...
        tr.addEventListener('click', () => { listSelectItem(tr.dataset.hash) })
        ...
        else if(item.type === 'image') imgFileIcon.src = "/static/icon/file/IMG.png"
...
function listFooterListing(list) {
    const main = document.querySelector('#list > main')
    ...
    if(recents.length) {
        recents.sort((a, b) => {
            return (new Date(b.recent_read_at)).getTime() - (new Date(a.recent_read_at)).getTime()
        })
        localStorage.setItem('recents', JSON.stringify(recents))
        const recent = recents[0]

        const footerFileicon = footer.querySelector('td.file-icon img')
        if(recent.type === 'text') footerFileicon.src = "/static/icon/file/TXT.png"
        else if(recent.type === 'image') footerFileicon.src = "/static/icon/file/IMG.png"
        else if(recent.type === 'pdf') footerFileicon.src = "/static/icon/file/PDF.png"
        ...
        tableFooter.addEventListener('click', () => { listSelectItem(tableFooter.dataset.hash) })
...
function listSelectItem(hash) {
    try {
        localStorage.setItem('contents', hash)
...
async function recentPageLoad() {
    try {
        ...
        for(recent of recents) {
            ...
            else if(recent.type === 'image') imgFileIcon.src = "/static/icon/file/IMG.png"
            else if(recent.type === 'pdf') imgFileIcon.src = "/static/icon/file/PDF.png"
