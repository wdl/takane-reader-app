...
async function contentsCheckPos() {
    try {
        ...
        const result = await capi({
            method: 'GET',
            url: `/${sectionContents.dataset.hash}/contents/pos`
        })
...
async function contentsPageLoad(prevLoad = false) {
    try {
        ...
        const spanInfoTitle = document.querySelector('#contents > .info-box > .top > .title > label')
...
async function contentsPrev() {
    try {
        ...
        if(!articlePrev.classList.contains('loading')) {
            ...
            capi({
                method: 'PUT',
                url: `/${sectionContents.dataset.hash}/contents/${articlePrev.dataset.index}`
            })
...
async function contentsNext() {
    try {
        ...
        if(!articleNext.classList.contains('loading')) {
            ...
            capi({
                method: 'PUT',
                url: `/${sectionContents.dataset.hash}/contents/${articleNext.dataset.index}`
            })
...
async function contentsToggleInfo(force = false) {
    try {
        ...
        const infoBottom = document.querySelector('#contents > .info-box > .bottom')
...
document.querySelector('#contents .touch-box').addEventListener('touchstart', (e) => {
...
document.querySelector('#contents .touch-box').addEventListener('touchend', (e) => {
    const pageX = e.pageX || e?.touches[0]?.pageX || e?.changedTouches[0]?.pageX || e?.originalEvent?.touches[0]?.pageX || e?.originalEvent?.changedTouches[0]?.pageX
    const pageY = e.pageY || e?.touches[0]?.pageY || e?.changedTouches[0]?.pageY || e?.originalEvent?.touches[0]?.pageY || e?.originalEvent?.changedTouches[0]?.pageY 
    ...
    if(startX && startY) {
        ...
        if(Math.abs(moveX) + Math.abs(moveY) > 30) {
            ...
            const quadrant = (((moveY < 0 ? moveX : -1 * moveX) / Math.abs(moveX)) / -2) + moveY / Math.abs(moveY) + 2.5
...
if (matchMedia('(pointer:fine)').matches) {
    document.querySelector('#contents .touch-box .left').addEventListener('click',  (e) => { contentsPrev() })
    document.querySelector('#contents .touch-box .center').addEventListener('click', (e) => { contentsToggleInfo() })
    document.querySelector('#contents .touch-box .right').addEventListener('click', (e) => { contentsNext() })
}

function contentsRangeCalc() {
    const sectionContents = document.querySelector('#contents')
    const articleThis = document.querySelector('#contents > .contents-box > .this')
    const inputRange = document.querySelector('#contents > .info-box > .bottom > .center > input')
    const percent = articleThis.dataset.index / sectionContents.dataset.length * 10000
...
function contentsRangeInput(inputRange) {
    const labelRange = document.querySelector('#contents > .info-box > .bottom > .left > label') 
	inputRange.style.background = `linear-gradient(to right, #4BD663, #4BD663 ${inputRange.value / 100}%, white ${inputRange.value / 100}%)`;
...
async function contentsRangeChange(inputRange) {
    try {
        ...
        const imgBackOriginRange = document.querySelector('#contents > .info-box > .bottom > .right > img')
        imgBackOriginRange.dataset.origin = articleThis.dataset.index
        imgBackOriginRange.classList.add('active')

        let thisPos = Math.round(inputRange.value * Number(sectionContents.dataset.length) / 10000)
        ...
        await capi({
            method: 'PUT',
            url: `/${sectionContents.dataset.hash}/contents/${thisPos}`
        })
...
async function contentsReload() {
    try {
        const reloadingBox = document.querySelector('#contents > .reloading-box')
        const reloadingBoxImage = document.querySelector('#contents > .reloading-box > img')
