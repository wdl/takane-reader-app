...
async function contentsImageArticlePrevLoad() {
    try {
        ...
        img.src = `https://api.text.takane.me/${sectionContents.dataset.hash}/contents/${indexPrev}?token=${localStorage.getItem('access_token')}`
...
async function contentsImageArticleThisLoad() {
    try {
        ...
        const img = document.createElement('img')
        img.classList.add('contents-image')
        img.src = `https://api.text.takane.me/${sectionContents.dataset.hash}/contents/${indexThis}?token=${localStorage.getItem('access_token')}`
...
async function contentsImageArticleNextLoad() {
    try {
        ...
        const img = document.createElement('img')
        img.classList.add('contents-image')
        img.src = `https://api.text.takane.me/${sectionContents.dataset.hash}/contents/${indexNext}?token=${localStorage.getItem('access_token')}`
